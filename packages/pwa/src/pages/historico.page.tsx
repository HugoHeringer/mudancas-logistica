      ...
      <div className="p-4 grid grid-cols-2 gap-3">
        <Card>
          <CardContent className="p-3 text-center">
            ...
            <p className="text-xs text-muted-foreground/60">Concluídas</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3 text-center">
            ...
            <p className="text-xs text-muted-foreground/60">Horas cobradas</p>
          </CardContent>
        </Card>
      </div>
        ...
        {(['todas', 'concluida', 'cancelada'] as const).map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
              filtro === f ? 'bg-night text-cream' : 'bg-muted text-muted-foreground hover:bg-muted/50'
            }`}
          >
        ...
        {isLoading ? (
          ...
          mudancasFiltradas.map((mudanca: any) => (
            <Link key={mudanca.id} to={`/mudanca/${mudanca.id}`}>
              <div className="bg-card/80 rounded-lg shadow-sm p-4 space-y-2 border-l-4 border-l-border transition-shadow hover:shadow-md">
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-foreground">{mudanca.clienteNome}</h2>
                ...
                <div className="flex items-center gap-2 text-muted-foreground text-sm">
                  ...
                  {mudanca.conclusao?.horasCobradas && (
                    <span className="text-muted-foreground/40">• {mudanca.conclusao.horasCobradas}h</span>
                  )}
                </div>
                <div className="flex items-center gap-2 text-muted-foreground text-sm">
