  ...
  return (
    <div className="min-h-screen bg-sand pb-20">
      ...
      <header className="bg-night text-cream p-4">
        ...
        {/* Active indicator */}
        {temMudancaAtiva && (
          <div className="mt-2 bg-gold/20 border border-gold/30 rounded-lg px-3 py-1.5 text-sm font-medium text-gold">
      ...
      {/* Date selector */}
      <div className="bg-sand-dark border-b border-sand-medium px-2 py-3">
              ...
              <button
                ...
                className={`flex flex-col items-center min-w-[52px] px-2 py-1.5 rounded-lg transition-all duration-200 ${
                  isSelected
                    ...
                    : 'text-brown-medium hover:bg-sand-medium/50'
                }`}
              >
      ...
      <main className="p-4 space-y-3">
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div key={i} className="bg-cream/60 rounded-lg p-4 animate-pulse">
                ...
                <div className="h-4 bg-sand-medium rounded w-3/4 mb-2" />
                <div className="h-4 bg-sand-medium rounded w-2/3" />
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-12 text-brown-medium/60">
            ...
            <p className="font-medium text-brown">Sem mudanças agendadas</p>
          ...
          mudancasDoDia.map((mudanca: any) => (
            <Link key={mudanca.id} to={`/mudanca/${mudanca.id}`}>
              <div className={`bg-cream/80 rounded-lg shadow-sm p-4 space-y-2 border-l-4 transition-shadow hover:shadow-md ${
                mudanca.estado === 'a_caminho' ? 'border-l-gold' :
                mudanca.estado === 'em_servico' ? 'border-l-night' :
                mudanca.estado === 'concluida' ? 'border-l-terracotta' :
                'border-l-gold-light'
              } ${mudanca.id === proximaMudanca?.id ? 'ring-2 ring-gold/40 shadow-gold/10' : ''}`}>
                {mudanca.id === proximaMudanca?.id && (
                  <div className="flex items-center gap-1.5 text-gold text-[10px] font-bold tracking-wider uppercase mb-1">
                ...
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-lg text-brown">{mudanca.clienteNome}</h2>
